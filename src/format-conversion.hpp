...
static inline enum audio_format get_obs_format(WAVEFORMATEX *format)
{
	...
	case WAVE_FORMAT_EXTENSIBLE:
		// SubFormat lives past the end of a plain WAVEFORMATEX; only read it
		// when the extension block is actually present.
		if (format_ex->Format.cbSize < sizeof(WAVEFORMATEXTENSIBLE) - sizeof(WAVEFORMATEX))
			return AUDIO_FORMAT_UNKNOWN;
