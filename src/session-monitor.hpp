...
public:
	...
	STDMETHOD(OnDeviceAdded)(LPCWSTR device_id)
	{
		...
		if (!PostThreadMessageA(worker_tid, SessionEvents::DeviceAdded,
	...
	{
		auto id = new std::wstring(device_id);
		if (!PostThreadMessageA(worker_tid, SessionEvents::DeviceRemoved,
					reinterpret_cast<WPARAM>(id), NULL))
			delete id;
		return S_OK;
	}
	...
	STDMETHOD(OnDeviceStateChanged)(LPCWSTR device_id, DWORD new_state)
	{
		...
		if (!PostThreadMessageA(worker_tid, code, reinterpret_cast<WPARAM>(id), NULL))
			delete id;
...
public:
	...
	STDMETHOD(OnSessionDisconnected)(AudioSessionDisconnectReason reason)
	{
		...
		if (!PostThreadMessageA(worker_tid, SessionEvents::SessionExpired,
					reinterpret_cast<WPARAM>(session_key_msg), NULL))
			delete session_key_msg;
		return S_OK;
	}
...
public:
	...
	// The notification client member is registered with WASAPI by address, so
	// this object must never be copied or moved.
...
public:
	...
	// Registered with WASAPI by address; must never be copied or moved.
